...
namespace rosbag2_generic_topic
{
    ...
    std::shared_ptr<GenericPublisher>
        Rosbag2Node::create_generic_publisher(const std::string& topic, const std::string& type, const rclcpp::QoS& qos)
    {
        library_generic_publisher_ = rosbag2_cpp::get_typesupport_library(type, "rosidl_typesupport_cpp");
        auto type_support =
            rosbag2_cpp::get_typesupport_handle(type, "rosidl_typesupport_cpp", library_generic_publisher_);
        return std::make_shared<GenericPublisher>(get_node_base_interface().get(), *type_support, topic, qos);
    }
    ...
    {
        library_generic_subscriptor_ = rosbag2_cpp::get_typesupport_library(type, "rosidl_typesupport_cpp");
        auto type_support =
            rosbag2_cpp::get_typesupport_handle(type, "rosidl_typesupport_cpp", library_generic_subscriptor_);
        ...
        {
            RCLCPP_ERROR_STREAM(get_logger(), "Error subscribing to topic '" << topic << "'. Error: " << ex.what());
        }
    ...
    std::shared_ptr<rcutils_string_map_t> get_initialized_string_map()
    {
        ...
        *substitutions_map = rcutils_get_zero_initialized_string_map();
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("rosbag2_node"), "Failed to initialize string map within rcutils.");
        ...
        return std::shared_ptr<rcutils_string_map_t>(
            ...
            [](rcutils_string_map_t* map)
            {
                ...
                {
                    RCLCPP_ERROR(
                        rclcpp::get_logger("rosbag2_node"),
                        "Failed to deallocate string map when expanding topic.");
                }
            });
    }
    ...
    std::string Rosbag2Node::expand_topic_name(const std::string& topic_name)
    {
        ...
        {
            RCLCPP_ERROR(get_logger(), "Failed to initialize string map within rcutils.");
            return "";
        }
        rcl_ret_t ret = rcl_get_default_topic_name_substitutions(substitutions_map.get());
        if (ret != RCL_RET_OK)
        {
            RCLCPP_ERROR(get_logger(), "Failed to initialize map with default values.");
            return "";
        }
        ...
        if (ret != RCL_RET_OK)
        {
            RCLCPP_ERROR_STREAM(
                get_logger(),
                "Failed to expand topic name " << topic_name << " with error: " << rcutils_get_error_string().str);
            return "";
        }
    ...
    std::unordered_map<std::string, std::string>
        Rosbag2Node::get_topics_with_types(const std::vector<std::string>& topic_names, bool include_hidden_topics)
    {
        ...
        {
            if (std::find(sanitized_topic_names.begin(), sanitized_topic_names.end(), topic_and_type.first) !=
    ...
    std::unordered_map<std::string, std::string> Rosbag2Node::get_all_topics_with_types(bool include_hidden_topics)
    {
        return filter_topics_with_more_than_one_type(this->get_topic_names_and_types(), include_hidden_topics);
    }
